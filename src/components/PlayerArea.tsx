...
export function PlayerArea({ player, isCurrentPlayer, label, isTop }: PlayerAreaProps) {
  ...
  return (
    <div className={`flex items-center gap-3 px-3 py-1.5 rounded-lg border transition-colors ${
      isCurrentPlayer ? 'bg-[#1a1508]/50 border-[#c9a84c]/30' : 'bg-[#0f0f18]/50 border-gray-800/30'
    }`}>
      {/* Avatar */}
      <div className={`rounded-full flex items-center justify-center shrink-0 border ${
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          <span className="font-heading text-white font-bold" style={{ fontSize: 'clamp(10px, 1.1vw, 14px)' }}>{label}</span>
          {isCurrentPlayer && <span className="text-[#f0d68a] animate-pulse font-heading" style={{ fontSize: 'clamp(8px, 0.8vw, 11px)' }}>⚡</span>}
        </div>

        {/* Health bar */}
        <div className="w-full bg-gray-800 rounded-full overflow-hidden border border-gray-700/30"
          style={{ height: 'clamp(12px, 1.4vw, 18px)' }}
          title={`Здоровье: ${player.health}/${player.maxHealth}`}>
          <div className={`h-full ${healthColor} transition-all duration-500 rounded-full flex items-center`}
            style={{ width: `${healthPercent}%`, minWidth: player.health > 0 ? '20px' : '0' }}>
            <span className="font-heading font-bold text-white drop-shadow px-1 whitespace-nowrap"
        ...
        <div className="flex items-center gap-1 mt-0.5">
          ...
          <span className="text-blue-300 font-heading font-bold" style={{ fontSize: 'clamp(8px, 0.9vw, 11px)' }}>
      ...
      {/* Stats */}
      <div className="flex flex-col items-end gap-0.5 text-gray-500 shrink-0 font-body"
