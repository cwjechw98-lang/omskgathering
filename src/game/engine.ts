...
export function getEffectiveAttack(card: CardInstance, player: PlayerState, opponent?: PlayerState): number {
    ...
    case 'mer_omska': {
      for (let i = 0; i < 2 && player.field.length < 7; i++) {
        const chinovnik = createCardInstance({
          id: 'chinovnik', name: 'Чиновник', cost: 0, color: 'white',
          type: 'creature', attack: 1, health: 1, description: 'Токен.',
          flavor: '«Приходите завтра.»', emoji: '👔', keywords: [], rarity: 'common'
        });
...
function applySpellEffect(
  ...
  switch (card.data.id) {
    ...
    case 'yama_na_doroge': {
      const targets = opponent.field.filter(c => getEffectiveAttack(c, opponent) <= 3 && !hasKeyword(c, 'hexproof'));
      if (targets.length > 0) {
    ...
    case 'ne_pokiday_omsk': {
      ...
      if (targets.length > 0) {
        ...
        const sorted = targets.sort((a, b) =>
          getEffectiveAttack(b, opponent) - getEffectiveAttack(a, opponent));
    ...
    case 'norminette': {
      const targets = opponent.field.filter(c => getEffectiveAttack(c, opponent) <= 4 && !hasKeyword(c, 'hexproof'));
      if (targets.length > 0) {
    ...
    case 'siberian_gnev': {
      if (opponent.field.length > 0) {
        const sorted = [...opponent.field].sort((a, b) =>
          getEffectiveAttack(b, opponent) - getEffectiveAttack(a, opponent));
...
export function attackCreature(
  ...
  newState.log.push(`⚔️ ${attacker.data.name} (${atkDamage}⚔) vs ${defender.data.name} (${defDamage}⚔)`);
...
export function endTurn(state: GameState): GameState {
  ...
  newState.log.push(`🔄 Ход ${newState.turnNumber}: ${nextPlayerKey === 'player1' ? 'Игрок' : 'ИИ Омска'}`);
