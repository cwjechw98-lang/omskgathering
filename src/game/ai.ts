...
import { playCard, attackPlayer, attackCreature, endTurn, getEffectiveAttack, getEffectiveHealth } from './engine';
...
export function aiTurn(state: GameState): { state: GameState; comment: string | null; actions: AIAttackAction[] } {
  ...
  const bypassDamage = attackers.filter(canBypassDefenders)
    .reduce((s, c) => s + getEffectiveAttack(c, gs.player2, gs.player1), 0);

  const isLethal = defenders.length === 0 &&
    attackers.reduce((s, c) => s + getEffectiveAttack(c, gs.player2, gs.player1), 0) >= gs.player1.health;
...
function scoreCardToPlay(card: CardInstance, ai: PlayerState, enemy: PlayerState): number {
  const enemyThreat = enemy.field.reduce((s, c) => s + (c.currentAttack || 0) * 1.5 + (c.currentHealth || 0) * 0.5, 0);
  const aiFieldStrength = ai.field.reduce((s, c) => s + (c.currentAttack || 0) + (c.currentHealth || 0) * 0.5, 0);
  ...
  if (card.data.type === 'creature') {
    ...
    if ((card.keywords.includes('defender') || card.keywords.includes('vigilance')) && enemyThreat > 4) s += 15;
    if ((card.keywords.includes('defender') || card.keywords.includes('vigilance')) && aiHealthRatio < 0.5) s += 20;
...
function findBestTarget(
  att: CardInstance, atkPow: number, atkHp: number,
  targets: CardInstance[], aiPlayer: PlayerState, enemyPlayer: PlayerState
): CardInstance | null {
  ...
  for (const t of targets) {
    ...
    let score = 0;
    if (canKill && willSurvive) score = 100 + (t.data.cost || 0) * 5; // Best: kill and survive
    else if (canKill) score = 50 + (t.data.cost || 0) * 3; // OK: kill but die
    else if (willSurvive) score = 30; // Chip damage, survive
...
function findBestTrade(
  att: CardInstance, atkPow: number, atkHp: number,
  enemies: CardInstance[], aiPlayer: PlayerState, enemyPlayer: PlayerState
