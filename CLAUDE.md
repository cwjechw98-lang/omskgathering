...
Харнесс — DeepSeek Harness (DSH), не Claude Code. Инструмента `Task`, типов агентов
`Explore`/`Plan`/`general-purpose` и MCP-инструментов вида `mcp__*` здесь НЕТ. Не ищи их.

### Делегирование

- `subagent` — отдельный агент со своим контекстом. Историю этого диалога он не видит,
  поэтому в задание вкладывай всё необходимое. Работает в фоне, результат приходит отдельно.
- `subagent_fork` — дочерний агент, который видит завершённые ходы этого диалога.
- `workflow` — JS-сценарий для веера однотипных подзадач (аудит многих файлов, миграция).
  Для одной-двух задач бери обычный `subagent`, не разворачивай workflow.

**Правило:** задача делится на 2+ независимые части → делегируй их параллельно.

### Скиллы

Лежат в `C:\Users\katoc\.dsh\skills\`, подгружаются инструментом `skill` только при вызове —
в промпте занимают одну строку каталога. Актуальный список смотри в каталоге сессии, а не
здесь: он меняется.

Для этого проекта чаще всего полезны: `verification-before-completion` (перед заявлением
о готовности), `commit-conventions` (сообщения коммитов), `test-driven-development`,
`systematic-debugging`.

### Ветка

Работаем в `main`, фича-ветки не создаём. Пуш в `main` запускает GitHub Actions
(`quality-gate.yml`, `deploy-pages.yml`).

### Перед началом любой задачи

1. Прочитай описание задачи целиком.
2. Делится на независимые части? Делегируй.
3. Собери доказательства **до** заявления о готовности: вывод команды или наблюдаемое
   поведение, а не «должно работать».
