...
- **Как проверял — и почему не vitest.** `vitest run` и `vite build` под файловой песочницей этой сессии падают с `spawn EPERM`: esbuild не может поднять дочерний процесс, а расширенный доступ для этого вызова не одобрен. Поэтому проверка сделана двумя скриптами на нативном Node 24 (он исполняет TypeScript сам, а JSX — через загрузчик `scripts/tsx-loader.mjs` на самом `typescript`, без esbuild):
  - `npm run check:tutorial-lessons` — **19/19** проверок: порядок уроков, все три состояния, границы показа, сквозной прогон боя 1→2→2→3→3→4 без пропусков;
  - `npm run check:tutorial-render` — **18/18** проверок: отрисовка панели в строку через `react-dom/server` — номер шага, заголовок, текст причины, подсветка зоны.
  Эти скрипты **не заменяют** `vitest`, а закрывают невозможность его запустить локально; те же сценарии продублированы в тестах и проверяются в CI.
- **Quality Gates локально:** `eslint src/` **0 errors** / 3 warnings (было 5 errors), `tsc --noEmit` **0 ошибок**, `check:tutorial` 37/37. `vitest` и `build` локально недоступны по причине выше — их прогоняет CI (`quality-gate.yml`).
