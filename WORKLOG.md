...
### 2026-09-22 — баланс цветов: зелёный не умел ничего, и устаревший снимок данных

- **Порядок работ выбран сознательно.** Шаг 4 утверждённого плана (проверка баланса цветов) не выполнялся ни разу, и он обязан идти раньше цветной маны и кампании: цвет уже стал выбором игрока, а если цвета неравны по силе, выбор фиктивный — есть один правильный цвет, и это ровно та болезнь, которую лечили. Цветная мана такой перекос только усилит, а кампания — контент поверх правил, и писать его на незастывших правилах значит переделывать.
- **Первым делом прибор соврал, и это оказалось главным за день.** Новый отчёт показал бесцветный цвет как **одну** карту вместо тринадцати. Причина: `reports/cards.json` — снимок `src/data/cards.ts`, его читают больше десяти скриптов, и **ничто его не пересобирает**. Снимок был от 19:11, а цвета я менял в 19:41. То есть отчёт уверенно измерил прошлое состояние игры. Теперь устаревший снимок — это отказ с кодом 1, а не тихий результат: `reports/cards.json` старше `cards.ts` → «пересобери: npm run jev:cards». Проверено прогоном: со старым снимком отказ, после пересборки проходит.
- **Второй раз прибор соврал уже в моей арифметике.** Он объявил белый цвет слабейшим (0.73). Настоящая причина — деление на ноль: у Чиновника цена 0 маны, нормы у него нет, и он попадал в среднее как отношение 0, топя весь цвет. Токен не выбирают при сборке колоды, поэтому в сравнение цветов он не входит. После починки белый поднялся до 0.85, и вывод «белый слабейший» исчез.
- **Третий раз — ложный ноль в подсчёте ответов.** У Росгвардии в тексте «уничтож**ить**», а я искал «уничтож**ьте**», и белый получил ноль убийств, которых у него нет. Искать по корню слова.
- **Разброс по статам оказался неизмеримым, и отчёт это честно пишет.** Проверил чувствительность к нашим же догадкам: менял цену ключевых слов по одной в восьми вариантах. Устойчиво только одно — зелёный первый по статам во всех восьми, но лучшие существа за ману и есть характер зелёного, это замысел. Порядок остальных цветов переворачивается от смены одной догадки: чёрный гуляет с третьего места на последнее. Разброс 1.20×–1.40× — это разброс наших допущений, а не измерение игры. Прибор, который не может ответить, обязан это сказать, а не выдать среднее за истину.
- **Настоящая находка — не в статах, а в возможностях.** Классы эффектов уже размечены судьёй, я только разложил их по цветам (без новых обращений к модели). У **зелёного оказалось 0 убийств и 0 остановок** — ни одного способа вмешаться в чужое существо. Одиннадцать карт: усиления, лечение, добор, растоптать. Проверено чтением самих текстов, не счётчиком.
- **Почему это критично именно у нас.** В движке **нет шага блокирования**: атакует нападающий и сам выбирает цель. В MTG зелёный выживает без убийств потому, что его крупные звери **блокируют**; здесь зверь 6/6 не мешает существу 2/2 бить в лицо. Значит зелёный не мог ни защититься, ни убить — только гонка. Для синего ноль убийств при шести заморозках и защитниках — это его характер, задержка; для зелёного ноль был дырой.
- **Правка минимальная и бесплатная.** «Бабка с Семечками» (2 маны, 0/3) получила **Защитник**. Она стоила этого изменения ноль: атака у неё 0, нападать она не может в принципе, так что перехват не отнимает у неё ничего и даёт зелёному единственный ответ. Её реплика Хранителя и так говорила ровно это: «Бабка никуда не уедет — у неё лавочка». Механика догнала текст.
- **Проверено, что «Защитник» — это правда перехват**, а не украшение: `engine.impl.ts:175-183` и `ai.ts:183-184` реализуют его как Taunt, и это покрыто тестами (`engine.combat.test.ts:31`). Заодно выяснилось, что во всей игре только два существа с нулевой атакой — Бабка и Кофемашина, — и оба теперь защитники, так что бессмысленных атакующих нет.
- **Новый сторож `tests/game/color-answers.test.ts`** (8 проверок): у каждого цвета есть хотя бы один ответ на чужое существо, и существо с нулевой атакой обязано быть защитником. **Сторож проверен на падение:** убрал защитника — упали три проверки с точными сообщениями, вернул — проходят. Тест, который проходит всегда, ничего не стоит.
- **Новые инструменты:** `scripts/jev/color-balance.mjs` (`npm run jev:color-balance`) и отчёт `reports/color-balance.md`.
- **Остаётся открытым:** зелёный по-прежнему единственный цвет без убийств (1 остановка против 6 у синего). Это может быть его характером — «выживи и перерасти», — но это решение о замысле, а не измерение, и его должен принять человек.
- **Quality Gates:** `tsc --noEmit` 0 ошибок, `eslint src/` 0 errors / 3 warnings, `vitest` **133/133** (+8 в `tests/game/color-answers.test.ts`), регрессия боя 54/54, обучение 18/18, сборка OK. Коммит `eac1b38`.
- **Следующий шаг:** цветная мана (шаг 5), затем кампания из пяти битв (шаг 6).

### 2026-09-22 — цветная мана: земля даёт ману своего цвета, а колода по умолчанию оказалась вне правил игры

- **Что было не так.** Мана была одним числом, а цвет карты влиял только на цвет рамки: любую карту платила любая земля. Цвет существовал в конструкторе колод как фильтр, но в самой игре его не было. Шаг 5 делает цвет настоящим: земля даёт ману **своего** цвета, и карта требует ману своего цвета.
- **Первым делом измерил, а не стал менять.** Оказалось, что рампа уже MTG-шная: `maxMana` растёт **только** от разыгранных земель, одна земля в ход, автоматического +1 за ход нет. То есть цветной пул ложится сюда без переделки экономики — земля и так единственный источник маны. Поверхность тоже оказалась маленькой: 28 мест во всём `src/game` знали про ману.
- **Правило оплаты — одно предложение.** Чтобы сыграть карту цвета X за N маны, нужно минимум `pipCount(N)` маны цвета X, остаток можно платить любой маной. `pipCount` = 1 для дешёвых (до 3 маны) и 2 для дорогих (4 и больше).
- **Почему не «вся мана должна быть цвета карты».** Это правило убило бы двухцветные колоды. В колоде из двух цветов земель каждого примерно половина, поэтому карту за 5 маны своего цвета там не разыграть почти никогда. В MTG моно-цветная карта за 5 маны обычно стоит «3GG» — два зелёных пипса, а не пять; наше правило повторяет именно это. Жёсткий вариант я рассматривал и отбросил по арифметике, а не по вкусу.
- **Главная находка дня — колода по умолчанию нарушала правила самой игры.** `createDeck()` собирала **233 карты всех шести цветов** и по 16 земель каждого. А конструктор колод (`DeckBuilder.tsx`) учит другому: рекомендуемый размер 40, максимум 2 цвета, земель не меньше 35%. То есть быстрый бой шёл не по тем правилам, которым игра учит.
- **С цветной маной эта колода не играет вообще, и это считается.** Земель каждого цвета в ней одна шестая от 96, то есть к пятому ходу нужного цвета обычно **нет вовсе**. Проверил арифметикой, а не на глаз: при 41% земель и шести равных долях на восьмом ходу в среднем 1.33 земли на цвет — карту с двумя пипсами не оплатить.
- **Новая колода: 40 карт, два цвета, 14 земель.** Белый и зелёный — самые «дружелюбные» цвета (порядок, лечение, защита и большие существа), на них проще провести первую партию. Проверено прогоном: 40 карт, цвета ровно `white,green`, 14 земель (7+7), кривая `1,1,1,1,2×10,3×6,4,4,5,6,8` — четыре карты за одну ману и десять за две, то есть колода играет с первого хода. `createDeck(['blue','black'])` тоже даёт 40 карт и 14 земель.
- **Бесцветная мана стала общей.** Площадь Бухгольца даёт бесцветную ману, и она платит за любую карту в части «остаток» — как generic в MTG. Так у бесцветной земли появилась настоящая роль, а не только +1 здоровье. Бесцветные карты Школы 21 не требуют пипсов вообще: они платятся любой маной. Это ровно их характер — гибкая оплата в обмен на слабейшие статы в игре (0.78 по замеру шага 4).
- **Мана от существ и чар — отдельный пул «любая».** Писинер и Святой Граф дают ману, которая не принадлежит цвету; она платит за что угодно, включая цветные пипсы. Иначе пришлось бы решать неразрешимый вопрос «какого цвета мана у программиста».
- **Новый модуль `src/game/mana.ts`** — чистые функции оплаты, отдельно от движка: `payMana`, `canPay`, `pipsFor`, `pipCount`, `refillPool`, `describePool`. Оплата возвращает **новый** пул или `null`, а не мутирует исходный: отказ в оплате не должен портить состояние.
- **Два поля маны — и это опасное место, которое сразу себя проявило.** `mana` остался числом (его читает интерфейс и дешёвые проверки), а платит теперь `manaPool`. Восемь тестов упали именно на этом: они ставили `player.mana = 10` и получали «ману, которую нельзя потратить». Я оставил оба поля ради размера правки, но закрыл риск иначе — тестами на то, что оплата не трогает исходный пул, и предупреждением в коде, что менять `mana` можно только вместе с `manaPool`.
- **Property-тест нашёл настоящую дыру, а не придирку.** Если пул не задан, `{...pool}` давал `undefined` в полях, `Math.min(undefined, n)` превращался в **NaN**, и функция возвращала пул из NaN вместо честного отказа. Починено в модуле: `{...emptyPool(), ...pool}`. Закреплено двумя тестами — на пустой объект и на частичный пул.
- **Поломку в регрессии не было видно обычным `tsc`.** `npm run test:regression` использует **свой** `tsconfig.regression.json`, поэтому ошибка типов в `tests/regression/combat-regression.ts` не всплыла при `npx tsc --noEmit`. Урок на будущее: ворот пять, и каждые надо запускать отдельно.
- **В регрессии тесты дают ману одним числом, а карты там разного цвета** (заклинания синие, чары белые, земли бесцветные). Решение — давать эту ману как пул «любая»: она платит за карту любого цвета, а общее число остаётся прежним, поэтому проверки «мана не потрачена при отказе» продолжили работать без единой правки. Иначе пришлось бы переписать два десятка мест по причине, к механике не относящейся.
- **Проверка «поле полно» переехала ДО оплаты.** Раньше мана списывалась, а потом возвращалась числом; с пулом возврат стал бы отдельной ошибкой. Теперь при полном поле карта просто не играется и мана не тратится — это и проверяет регрессия.
- **Конструктор колод: земли теперь входят в цвет колоды.** Раньше `colorsInDeck` их пропускал, потому что земля ни на что не влияла. Теперь зелёная земля в белой колоде честно делает её двухцветной. Бесцветная — исключение: она общая, не размывает колоду, и за неё игрока пугать нельзя.
- **Главное предупреждение конструктора — «цвет есть, а земли под него нет».** Красная строка перечисляет цвета карт, под которые в колоде нет ни одной земли: их физически нечем платить. До цветной маны такой проверки не существовало и существовать не могло.
- **Интерфейс показывает, из чего состоит мана.** В подсказке к счётчику — расшифровка пула («белая 2, зелёная 1»). Без неё игрок видит «💎3/5» и не понимает, почему зелёная карта не играется: общее число есть, а нужного цвета нет. Это первый вопрос, который возникает у человека при знакомстве с цветной маной.
- **ИИ обязан считать цвет, иначе он застревает.** Фильтр «играбельно» был `cost <= mana`; теперь это настоящая проверка оплаты, причём с учётом налога Бабушки с Метро (+1 к цене заклинания). Без учёта налога ИИ выбирал бы карту, которую движок откажется разыгрывать, и цикл розыгрыша вставал бы.
- **Новые тесты:** `tests/game/mana.test.ts` — 15 проверок правила оплаты (граница пипсов, отказ по цвету, мана «любая», отсутствие мутации, защита от вырожденного пула, сборка пула на ход).
- **Quality Gates:** `tsc --noEmit` 0 ошибок, `eslint src/` 0 errors / 3 warnings, `vitest` **149/149** (19 файлов, +15 на модуль маны), регрессия боя 54/54, обучение 20/20 и 18/18, сборка OK (7.32s).
- **Осталось по плану:** кампания из пяти битв (шаг 6) — Пророчество Птицы обещает битву магов, а победа пока ничего не меняет; и крючок «Бабушка с Метро» в сюжетном событии. Кампания получит колоды фракций — теперь, когда цвет стал механикой, у каждой фракции есть свой цвет, и это наконец означает разную игру, а не разную рамку.

### 2026-09-22 — прогонщик партий: цветная мана игру не сломала, но цвета оказались неравны втрое

- **Зачем понадобился замер.** Шаг 5 поменял цену каждой карты — не текст, а то, чем её платить. 149 тестов проверяют, что механика работает, но не отвечают на вопрос «осталась ли игра играбельной». Идти в кампанию в таком состоянии значило строить второй этаж, не проверив фундамент. Новый инструмент `scripts/sim/mana-sim.mjs` (`reports/mana-simulation.md`) отвечает числом: 1 600 партий ИИ против ИИ, зерно фиксировано, прогон воспроизводим.
- **Главная метрика — мёртвый ход:** ход, в котором на руке были карты, кроме земель, а не разыграно ни одной. Это и есть «не повезло с землями». Ориентир MTG — 10–15%.
- **Прибор соврал на первом же прогоне, и это главное событие дня.** Все зеркальные пары показали **0 побед первого игрока из 6**, хотя колоды у сторон одинаковые. Причина — моя ошибка: при подмене сторон (нужна, потому что `aiTurn` играет только за второго игрока) я переставлял игроков и очередь, но **забыл переставить метку победителя**. Движок объявлял победу в перевёрнутом мире, метка оставалась как есть, и первый игрок, выиграв все партии, записывался как проигравший все.
- **Поймал это не глазом, а противоречием в числах.** Первый игрок разыгрывал больше карт (10.5 против 8.7) и атаковал больше (11.3 против 8.8) — и «проигрывал» 0:6. Поведение противоречило итогу, значит врал прибор, а не игра. После починки зеркальные пары дают 47:33 при близком здоровье (16.2 против 10.7). Урок тот же, что и раньше: сначала подозревать инструмент.
- **Метрика проверена на срабатывание.** Заведомо сломанная мана-база (все 14 земель белые, зелёные карты платить нечем) поднимает долю мёртвых ходов с 16.4% до 65.1%. Метрика, которая не может поймать заведомую поломку, ничего не стоит.
- **Ответ на исходный вопрос: правило пипсов можно оставлять.** Решающее сравнение — одноцветные колоды, где цвет не стоит **ничего** (все земли подходят ко всем картам): белая 15.3%, зелёная 8.9%. Бело-зелёная колода по умолчанию — 16.4%. То есть цветные требования стоят **около одного процентного пункта**; остальные мёртвые ходы берутся из состава самой колоды (26 не-земель на 14 земель и дорогая кривая белого). Ослабление правила до одного пипса дало бы около пункта — не тот рычаг.
- **Настоящая находка оказалась не там, где я искал.** Круговой турнир одноцветных колод, 400 партий на цвет: чёрный **66.5%**, зелёный 61.0%, синий 57.8%, бесцветный 48.5%, белый 33.0%, красный **31.5%**. Разрыв 35.0% ± 3.3% — в десять раз больше ошибки. Это прямой замер поведения, и он **противоречит выводу шага 4** («разброс 1.20×–1.40× — это разброс наших допущений, а не измерение игры»). Шаг 4 мерил статы по нашей же таблице цен; здесь колоды играют друг с другом.
- **Чего этот замер не доказывает, и я это пишу в отчёте.** `createDeck` добирает колоду копиями самой дешёвой карты, поэтому у чёрного (10 разных карт) **8 карт по три копии**, а у белого (12 разных) — только 4. Колода из «трёшек» consistent-нее, и это само по себе преимущество. Значит 35% — разрыв **автособранных колод**, а не «силы цвета» в чистом виде; разделить вклад повторов и вклад самих карт этот прогон не может. Проверил, что дело не в настройке ИИ: жёсткие предпочтения по картам размазаны по цветам, больше всех у синего (7 карт, а он лишь третий), у белого четыре (а он последний).
- **Побочная находка: первый игрок получает слишком много.** В зеркальной паре он выигрывает 58.8% ± 5.5%. Причина видна в коде: `createInitialGameState` даёт ему **и первый ход, и добор** — шесть карт против пяти. В MTG первый игрок специально пропускает первый добор именно поэтому. Это было и раньше, просто не измерялось.
- **Что из этого следует для плана.** Правило пипсов оставить. Кампанию **нельзя** строить на `createDeck` по цветам фракций: автособранный чёрный вдвое сильнее автособранного красного, и фракция-чёрный будет бить фракцию-красный без игры. Колоды фракций собирать вручную либо сперва выровнять добор. Про первый ход решить отдельно.
- **Прибор падает громко.** Если после хода не переключилась очередь, скрипт бросает исключение: тихого мусора не будет. Отладочный скрипт, которым я искал причину, удалён — в нём оставалась та же ошибка с победителем, а сломанный прибор в репозитории оставлять нельзя.
- **Quality Gates:** `tsc --noEmit` 0 ошибок, `eslint src/` 0 errors / 3 warnings, `vitest` **149/149** (19 файлов), сборка OK (4.25s). Изменений в `src/` нет — только новый инструмент и отчёт.
