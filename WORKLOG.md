...
- **Известная проблема (не связана с merge):** в «Коллекции» карты без локального файла уходят в рантайме на `image.pollinations.ai`, что нарушает правило проекта «никогда не грузить картинки из внешних API в рантайме». Не замаплены `pisiner_21` и `khroniker_irtysha`.
